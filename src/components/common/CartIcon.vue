<template>
	<div class="cart-icon">
		...
		<div class="item-count">{{ bookCount }}</div>
	</div>
</template>
...
const { booksInCart } = storeToRefs(store);
const bookCount = computed(() => {
	return booksInCart.value.reduce(
		(acc, book) => acc + book.numberOfBooksOrdered,
		0
	);
});
</script>
